...
def delayhetsampling(
    ...
    ############################################################
    # Initialize delay estimates
    ############################################################

    # for c in clients:
    #     c["tau_est"] = c["compute_time"]
...
def async_fl(
    ...
    while current_time < MAX_TIME:
        ...
        ########################################################
        # Clients available this time slot
        ########################################################
        ...
        if len(available) > 0:
            ...
            for k in available:
                ...
                utility = (
                    c["quality"] /
                    (1.0 + beta * c["compute_time"])
                )

                scores.append((utility, k))

            scores.sort(reverse=True)

            selected = [
                k for _, k in scores[:min(topK, len(scores))]
            ]
        ...
        if current_time in arrivals:
            ...
            for k, delta, t0 in updates:
                ...
                utility = (
                    clients[k]["quality"] /
                    (1.0 + beta * clients[k]["compute_time"])
                )
            ...
            for utility, k, delta, t0 in ranked:
            ...
            for delta, alpha in zip(deltas, weights):
...
#             candidates = random.sample(available, min(m, len(available)))
...
def unified(clients,
    ...
    # Unified = PoC + Async (dropping quality)
    # ----------------------------------------------------------
