...
def create_non_iid_clients(
    ...
    label_indices = {
        i: (y_train == i).nonzero(as_tuple=True)[0]
        for i in range(C)
    }
    ...
    for k in range(K):
        ...
        for lbl in chosen_labels:
            idx = label_indices[int(lbl)]
            rand_idx = idx[torch.randint(len(idx), (per_label,))]
            X_list.append(X_train[rand_idx])
            y_list.append(y_train[rand_idx])
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots_cifar_noniid_log_regression_gpu/CIFAR_NONIID_LR_Test_accuracy.png", dpi=300, bbox_inches="tight")
#plt.show()
...
plt.grid(True)
plt.tight_layout()
plt.savefig("plots_cifar_noniid_log_regression_gpu/CIFAR_NONIID_LR_Training_accuracy.png", dpi=300, bbox_inches="tight")
#plt.show()

np.savez(
    f"results_seed_noniid_cifar_log_regression_{seed}.npz",
